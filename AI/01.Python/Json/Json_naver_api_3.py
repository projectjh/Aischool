...
def save_db(jsonResult):
    dbconn = sqlite3.connect('naver_blog.db')
    ...
    dbcursor.execute("""create table if not exists naver_blog(
                        ...
                        postdate text,
                        bloggername text)""")
    print("naver_blog.db SAVED")

    # insert db
    for row in jsonResult:
        dbcursor.execute("insert into naver_blog values (null, ?, ?, ?, ?, ?, ?)", (row['title'], row['description'], row['bloggerlink'], row['link'], row['postdate'], row['bloggername']))
