...
const ReviewCommentArticle = ({comment, setComment, view, setView}) => {
    ...
    const commentUpdate = (e) => {
        console.log('수정하려면 해당 댓글 인덱스가 필요해,', e.target.className, commentModifyRef.current.value);
    ...
    return (
        <div className={sessionIdx == comment.USER_IDX ? "CommentBox myCommentBox" : "CommentBox"}>
            <ul className={comment.COMMENT_IDX}>
                ...
                <li>{modify? <textarea ref={commentModifyRef}>{comment.COMMENT_TXT}</textarea> : comment.COMMENT_TXT }</li>
            ...
            {sessionIdx == comment.USER_IDX ? 
                ...
                {modify ? 
                    <div className="modifyOn">
                        <button onClick={commentUpdate} className={comment.COMMENT_IDX}>등록</button>
